...
if (isset($_POST['btnSave'])){
    ...
    if(count($err)==0){
      ...
      $sql="INSERT INTO  `house_categories`(name) VALUES ('$name')";
      $conn->query($sql);
      if($conn->affected_rows==1 && $conn->insert_id > 0){
        $success="category successfully inserted";
      }else{
        $error= "category inserted failed";
      }
    }
}
?>
...
<body>
  ...
  <?php } ?>    
        <div class="left">
            ...
            <div class="body1">
                <label>Name</label><br>
                <input type="text" id="name" name="name" placeholder="Enter category name" value="<?php echo isset($name)?$name:''?>"><br>
                    <?php
                    if(isset($err['name'])){?>
